...
<header class="bg-gradient-to-r from-green-600 to-teal-600 py-6 px-8 shadow-md rounded-lg mb-3 mt-4 mr-3">
    <div class="container mx-auto">
        ...
        <p class="text-white text-sm mt-1">User | Tambah Tanaman</p>
    </div>
</header>
    ...
    <div class="p-6 bg-white rounded-lg shadow-lg border border-gray-200">
        <form action="{{ route('tanamans.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="nama_tanaman" id="nama_tanaman" value="{{ old('nama_tanaman') }}"
            ...
            <div class="mb-4">
                ...
                <textarea name="deskripsi" id="deskripsi" rows="4"
                          class="block w-full mt-1 px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-green-500 focus:border-green-500">{{ old('deskripsi') }}</textarea>
                @error('deskripsi')
                    <span class="text-sm text-red-600">{{ $message }}</span>
                @enderror
            </div>
